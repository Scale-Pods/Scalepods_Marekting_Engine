    ...
    entries: [
      {
        ...
        steps: [
          ...
          'Single image: click an image to see it full size, click the checkmark in its corner to pick it. Carousel: every slide generates once — click "Regenerate this slide" on any tile that came out wrong, no need to redo the rest.',
      ...
      },
      {
        ...
        to: '/video-studio',
        icon: <Film size={17} />,
        what: 'Topic in, one continuous branded video out — either a motion-graphics explainer (on-screen text and shapes, effectively free) or real AI-generated video shots stitched together (a real cost per video, shown before you spend anything).',
        steps: [
          'Pick a type: "Motion graphics" (on-screen text, no AI video model) or "Generated clips" (real AI video, real cost — greyed out until this is enabled for your account).',
          'Pick what the video is about (a live trend, the strategy, or your own topic), the platform, shape, and slide/shot count. For generated clips, also pick the engine (Lite/Fast/Standard — cost per shot differs a lot) and see a rough cost estimate.',
          'Click "Draft brief" — AI writes the outline/storyboard AND the post caption together. Nothing has rendered or generated yet.',
          'Edit any slide (or shot prompt/duration), the caption, or the voiceover script. For generated clips, the exact cost is now shown on the Approve & Generate button itself.',
          'Click Approve & Render/Generate — for generated clips this opens a confirm step restating the real dollar amount first, since this is the one step in Video Studio that spends real money.',
          'The worker builds the video — stitching slides, or generating+stitching Veo shots — and burns in the logo. Takes a few minutes and keeps running if you navigate away. A failed generated-clips shot can be regenerated on its own without re-paying for the others.',
          'When it is done, preview the finished video and click "Send to Review" to hand it to Creative Review like any other post.',
        ],
        tip: 'Generated clips needs a Gemini API credential wired up before it can actually run — until then the option is visible but disabled, so you always know what is real right now.',
      },
      {
        id: 'creative-review',
