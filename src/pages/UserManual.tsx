    ...
    entries: [
      ...
      },
      {
        ...
        steps: [
          ...
          'Choose how far back each check searches (7 days by default, up to a year, or a custom number of days) and a region, including "Worldwide" for no region filter or a custom country code. Google News and RSS honor both exactly; Reddit, web search and YouTube currently stay at the last week regardless of the lookback you pick.',
    ...
    entries: [
      ...
      },
      {
        ...
        steps: [
          ...
          'Making something yourself? Use "Import" (next to Create Post) to bring a design in from your computer, Canva or Figma. It joins the queue as a draft, and you can pick who should review it right there.',
          'Finished a piece? Click the ⋯ on its tile (or "Send for review" inside the piece) and choose who should check it — only people who can approve are listed. They get a bell notification and an email, and it shows a small badge with their initials.',
          'Inside a piece, "Attachments for the reviewer" takes a file or a pasted link — a Google Doc or Drive link works well for briefs and copy. Only the person who added one, or an admin, can remove it.',
          'Checking work sent to you? Tap "Sent to me" to see just the pieces handed to you.',
        ],
        tip: 'Only people with full Creative Review access can approve or send back — designers and writers can hand work over, but never approve their own. Creating a Google Doc from inside the app is not available yet; paste the link to one you made instead.',
      },
    ],
  },
  {
    ...
    entries: [
      {
        ...
        steps: [
          ...
          'In List view, "Group by" splits the table into sections by status, assignee, reporter, priority or type — e.g. group by assignee and add a date range to see exactly what someone was given this week and where each item stands.',
          ...
          'A ticket’s Attachments tab holds proof of completed work — attach a file or paste a link. Anyone who can work the board can add one; removing one needs to be the person who added it, or an admin.',
    ...
    entries: [
      ...
      },
      {
        ...
        steps: [
          ...
          'Facebook takes a text post, one photo, several photos (a multi-photo post), or a video. When you pick a video and Facebook is selected, choose "Reel" (vertical, up to 90 seconds) or a plain "Video" beside it.',
          'Spotted a typo after it went live? Admins and the owner can open a published LinkedIn, Facebook or YouTube post in Recent activity and click "Edit live post" to change the caption (or, for YouTube, the title and description; for a Facebook video or Reel, its description). Every edit is recorded with who made it and whether the platform accepted it.',
        ],
        tip: '"Post now" is public and cannot be undone, so it always asks you to confirm first. YouTube video stays manual-only by design. Editing a live post: LinkedIn shows an "Edited" label to everyone and the image or video cannot change; Instagram, X and ScalePods Company Page posts cannot be edited from here (Instagram does not allow it, the other two go through Buffer), and the button says why. Facebook posts published before 25 Sep 2026 never actually went out and have no link; publish them again.',
      },
      {
        id: 'blog',
        label: 'Blog',
